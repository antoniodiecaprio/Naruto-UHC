...
public class Kisame extends NarutoRole {
    ...
    public void onPlayerDamageOnEntity(EntityDamageByEntityEvent event, Player player) {
        ...
        if(heartDamage >= 50) {
            ...
            player.sendMessage("§7▎ §fVous avez infligé §c50 coeurs §fde dégâts, vous obtenez donc §eAbsorption§f, §dRégénération §fet §6Résistance au Feu§f.");
        }
    ...
    public void onDistribute(Player player) {
        ...
        player.getInventory().addItem(new ItemBuilder(Material.BOOK).addUnsafeEnchantment(Enchantment.DEPTH_STRIDER, 3).toItemStack());
    }
    ...
    public void onAllPlayerDamageOnEntity(EntityDamageByEntityEvent event, Player damager) {
        if(Item.specialItem(damager.getItemInHand(), "Samehada")) {
            ...
            event.getEntity().sendMessage("§7▎ §cSamehada §fa utilisé son épée sur vous, vous ne pouvez plus utiliser vos pouvoirs pendant §c1 minute§f.");
            Bukkit.getScheduler().runTaskLater(narutoUHC, () -> blocked.remove(event.getEntity().getName()), 60*20);
        }
    }
